...
@push('scripts_bottom')
    ...
    <script>
        ...
        var notLoginToastTitleLang = 'not_login_toast_msg_lang';
    ...
    function fetchPurchase(qrCode, qrData) {
        ...
        return fetch('/api/development/qrcode', {
            ...
            body: JSON.stringify({
                code: qrCode,
                user_id: '{{ auth()->id() }}',
                bundle_id: '{{ $bundle->id }}', // Fallback to bundle_id if it's not available in the QR data
            })
        })
        .then(response => response.json())
        .then(result => {
