                    ...
                    @elseif($file->accessibility == 'paid')
                        @if(!empty($user) && $hasBought)
                            @if($file->downloadable)
                                <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getUrl() . '/file/' . $file->id . '/download' }}" 
                                   class="course-content-btns btn btn-sm btn-primary" 
                                   target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}">
                                    {{ trans('home.download') }}
                                </a>
                            @else
                                <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getLearningPageUrl() . '?type=file&item=' . $file->id }}" 
                                   target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" 
                                   class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('public.play') }}
                                </a>
                        ...
                        @else
                            <button type="button" class="course-content-btns btn btn-sm btn-gray disabled {{ empty($user) ? 'not-login-toast' : (!$hasBought ? 'not-access-toast' : '') }}">
                                {{ $file->downloadable ? trans('home.download') : trans('public.play') }}
                            </button>
                        @endif
                    @else
                        @if($file->downloadable)
                        ...
                        @else
                            ...
                            @elseif(in_array($file->storage, ['iframe', 'google_drive', 'dropbox']))
                                <a href="/course/{{ $course->slug }}/file/{{ $file->id }}/play" 
                                   target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" 
                                   class="course-content-btns btn btn-sm btn-primary">
                                    {{ trans('public.play') }}
                                </a>
