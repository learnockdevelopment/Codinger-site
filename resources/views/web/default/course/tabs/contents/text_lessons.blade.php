        ...
        <div class="d-flex align-items-center" href="#collapseTextLessons{{ $textLesson->id }}" aria-controls="collapseTextLessons{{ $textLesson->id }}" data-parent="#{{ $accordionParent }}" role="button" data-toggle="collapse" aria-expanded="true">
            ...
            @if($textLesson->accessibility == 'paid')
                @if(!empty($user) and $hasBought)
                    <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getLearningPageUrl() }}?type=text_lesson&item={{ $textLesson->id }}" target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" class="mr-15" data-toggle="tooltip" data-placement="top" title="{{ trans('public.read') }}">
            ...
            @else
                ...
                @else
                    <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getUrl() }}/lessons/{{ $textLesson->id }}/read" target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" class="mr-15" data-toggle="tooltip" data-placement="top" title="{{ trans('public.read') }}">
                        <span class="chapter-icon chapter-content-icon">
                            <i data-feather="file-text" width="20" height="20" class="text-gray"></i>
                        </span>
                    </a>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="">
                    ...
                    @elseif($textLesson->accessibility == 'paid')
                        @if(!empty($user) and $hasBought)
                            <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getLearningPageUrl() }}?type=text_lesson&item={{ $textLesson->id }}" target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" class="course-content-btns btn btn-sm btn-primary">
                                {{ trans('public.read') }}
                            </a>
                        @else
                    ...
                    @else
                        ...
                        @else
                            <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getUrl() }}/lessons/{{ $textLesson->id }}/read" target="{{ $shouldRedirectToMobileApp ? '_self' : '_blank' }}" class="course-content-btns btn btn-sm btn-primary">
                                {{ trans('public.read') }}
                            </a>
                        @endif
                    @endif
