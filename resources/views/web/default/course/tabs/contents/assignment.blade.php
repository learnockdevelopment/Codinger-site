            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="">
                    ...
                    @elseif(!empty($user) and $hasBought)
                        <a href="{{ session('error') ? '/mobile-app' : $course->getLearningPageUrl() }}?type=assignment&item={{ $assignment->id }}" target="{{ session('error') ? '_self' : '_blank' }}" class="course-content-btns btn btn-sm btn-primary">
                            {{ trans('public.read') }}
                        </a>
                    @else
