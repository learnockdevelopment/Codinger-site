<div class="row">
    ...
    @if(!empty($user) and $quiz->can_download_certificate and $hasBought)
        <a href="{{ session('error') ? '/mobile-app' : '/panel/quizzes/results/' . $quiz->result->id . '/showCertificate' }}" target="{{ session('error') ? '_self' : '_blank' }}" class="course-content-btns btn btn-sm btn-primary">
            {{ trans('home.download') }}
        </a>
    @else
