            ...
            @if(!empty($user) && $hasBought)
                <div class="d-flex align-items-center mt-20">
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" @if(($session->date < time()) || $sequenceContentHasError) disabled @endif id="sessionReadToggle{{ $session->id }}" data-session-id="{{ $session->id }}" value="{{ $course->id }}" class="js-text-session-toggle custom-control-input" @if(!empty($session->checkPassedItem())) checked @endif>
            ...
            <div class="d-flex align-items-center justify-content-between mt-20">
                ...
                <div class="">
                    @if($session->isFinished())
                        <button type="button" class="course-content-btns btn btn-sm btn-gray disabled flex-grow-1 session-finished-toast">{{ trans('public.finished') }}</button>
                    @elseif(empty($user))
                        <button type="button" class="course-content-btns btn btn-sm btn-gray disabled flex-grow-1 not-login-toast">{{ trans('public.go_to_class') }}</button>
                    @elseif($hasBought)
                        @if(!empty($checkSequenceContent) && $sequenceContentHasError)
                            <button
                                type="button"
                        ...
                        @else
                            <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : $course->getLearningPageUrl() . '?type=session&item=' . $session->id }}" target="_blank" class="course-content-btns btn btn-sm btn-primary flex-grow-1">{{ trans('public.go_to_class') }}</a>
                        @endif
                    @else
