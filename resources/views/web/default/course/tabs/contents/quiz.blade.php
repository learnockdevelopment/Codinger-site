    ...
    <div id="collapseQuiz{{ !empty($tagId) }}{{ $quiz->id }}" aria-labelledby="quiz_{{ $quiz->id }}" class="collapse" role="tabpanel">
        <div class="panel-collapse">
            <div class="d-flex align-items-center mt-20">
                ...
                <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                    <i data-feather="check-square" width="18" height="18" class="text-gray mr-5"></i>
                    <span class="line-height-1">{{ trans('update.attempts') }}: {{ (!empty($user) && !empty($quiz->result_count)) ? $quiz->result_count : '0' }}/{{ $quiz->attempt }}</span>
                </div>

                @if(!empty($user) && !empty($quiz->result_status))
                    <div class="d-flex align-items-center text-gray text-center font-14 mr-20">
                        <i data-feather="check-square" width="18" height="18" class="text-gray mr-5"></i>
            ...
            <div class="d-flex justify-content-end mt-20">
                @if(!empty($user) && $quiz->can_try && $hasBought)
                    <a href="{{ $shouldRedirectToMobileApp ? '/mobile-app' : '/panel/quizzes/' . $quiz->id . '/start' }}" class="course-content-btns btn btn-sm btn-primary">
