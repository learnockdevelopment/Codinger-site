...
<div class="webinar-card relative" data-aos="fade-up" data-aos-duration="1000" data-aos-once="true">
    ...
    <figure class="webinar-figure relative">
            ...
            <div class="badge-column">
                @if ($webinar->bestTicket() < $webinar->price)
                    <span class="badge badge-danger"
                        style="background: red;">{{ trans('public.offer', ['off' => $webinar->bestTicket(true)['percent']]) }}</span>
                ...
                @elseif($webinar->type == 'webinar')
                    ...
                    @elseif($webinar->isProgressing())
                        <span class="badge badge-secondary"
                            style="background: red;">{{ trans('webinars.in_progress') }}</span>
                    @else
                ...
                <div class="icon-column">
                    ...
                    <div>
                        <a href="/favorites/{{ $webinar->slug }}/toggle" id="favoriteToggle"
                            ...
                            <span class="badge bg-transparent d-flex flex-column align-items-center">
                                <i data-feather="heart"
                                    class="{{ !empty($isFavorite) ? 'favorite-active' : '' }} feather-solid"
        ...
        @if ($webinar->video_demo)
    <div id="webinarDemoVideoBtn"
        data-video-path="{{ $webinar->video_demo_source == 'upload' ? url($webinar->video_demo) : $webinar->video_demo }}"
        data-video-source="{{ $webinar->video_demo_source }}"
...
@endif
        ...
        <div class="webinar-image" style="background-image: url('{{ $webinar->getImage() }}');">
            ...
            <figcaption class="webinar-content">
                @if (!empty($webinar->category))
                    <span class="d-block font-14">{{ trans('public.in') }} <a
                            ...
                            class=" cat text-decoration-underline text-light">{{ $webinar->category->title }}</a></span>
                @endif
                <h3 class="webinar-title overflow-marquee"><a href="{{ $webinar->getUrl() }}">{{ clean($webinar->title, 'title') }}</a>
                </h3>
                ...
                <div class="">
                    ...
                    ">
                        <span class="badge bg-transparent d-flex flex-column align-items-center">
                            ...
                            <span>{{ $webinar->getSalesCount() }}</span>
                        </span>
                        <span class="badge bg-transparent d-flex flex-column align-items-center">

                            <span class="font-12">Duration</span>
                            <i data-feather="clock" width="20" height="20"></i>
                            <span>{{ convertMinutesToHourAndMinute($webinar->duration) }}</span>

                        </span>
                    </div>
                    <div class="d-flex justify-content-center mt-15">
                        <div>
                            <img src="{{ $webinar->teacher->getAvatar() }}"
                                style="
                    width: 40px;
                    height: 40px;
                    object-fit: cover;
                ...
                "
                        ...
                        <div class="d-flex justify-content-center flex-column">
                            <a class="webinar-teacher text-center webinar-teacher-name text-light text-decoration-underline"
                                href="{{ $webinar->teacher->getProfileUrl() }}"
                                target="_blank"href>{{ $webinar->teacher->full_name }}</a>
                            <p class="webinar-teacher text-center">{{ $webinar->teacher->bio }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="webinar-price py-3 d-flex justify-content-center">
                    @if (!empty($webinar->price) && $webinar->price > 0)
                        <span class="price">
                            @if ($webinar->price > $webinar->getPrice())
                                <span class="original-price">
                                    @php
                                        $originalPrice = handlePrice($webinar->price, true, true, false, null, true);
                                        $splitOriginalPrice = explode(' ', $originalPrice);
                                    @endphp
                                    <span class="price-value">{{ $splitOriginalPrice[0] }}</span>
                                    <span class="currency-symbol">{{ $splitOriginalPrice[1] ?? "$" }}</span>
                                </span>
                                <span class="discounted-price">
                                    ...
                                    @endphp
                                    <span class="price-value">{{ $splitDiscountedPrice[0] }}</span>
                                    <span class="currency-symbol">{{ $splitDiscountedPrice[1] ?? "$" }}</span>
                                </span>
                            @else
                                @php
                                ...
                                @endphp
                                <span class="price">
                                    <span class="price-value">{{ $splitNormalPrice[0] }}</span>
                                    <span class="currency-symbol">{{ $splitNormalPrice[1] ?? "$" }}</span>
                                </span>
                            @endif
                ...
                <a href="{{ $webinar->getUrl() }}" class="btn-modern">Enroll now</a>

            </figcaption>
        </div>

    </figure>
      @include('web.default.course.share_modal', ['course' => $webinar])
