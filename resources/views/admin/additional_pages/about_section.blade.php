...
@section('content')
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-12 col-md-8 col-lg-6">
                                    <form action="{{ getAdminPanelUrl() }}/additional_page/about_section/store" method="post">
                                        {{ csrf_field() }}
    ...
    <div class="input-group">
        ...
        <input type="text"
               name="value[link]"
               id="link"
               value="{{ (!empty($navbar_link)) ? $navbar_link->link : old('value.link') }}"
               class="form-control @error('value.thumbnail') is-invalid @enderror"/>
...
<div class="form-group">
                                            ...
                                            <input type="text" name="value[description]" value="{{ (!empty($navbar_link)) ? $navbar_link->description : old('value.description') }}" class="form-control  @error('value.description') is-invalid @enderror"/>
                                    ...
                                    @if(!empty($items))
                                        @foreach($items as $key => $val)
                                            <tr>
                                                ...
                                                <td>{{ $val['order'] }}</td>
                                                <td>
                                                    <a href="{{ getAdminPanelUrl() }}/additional_page/about_section/{{ $key }}/edit" class="btn-sm" data-toggle="tooltip" data-placement="top" title="{{ trans('admin/main.edit') }}">
                                                        <i class="fa fa-edit"></i>
                                                    </a>

                                                    @include('admin.includes.delete_button',['url' => getAdminPanelUrl().'/additional_page/about_section/'. $key .'/delete','btnClass' => 'btn-sm'])
                                                </td>
                                            </tr>
                                        @endforeach
                                    @endif
