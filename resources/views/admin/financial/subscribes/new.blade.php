...
@section('content')
    ...
    <textarea id="summernote" name="description " class="summernote form-control @error('content')  is-invalid @enderror"
              value="{{ !empty($subscribe) ? $subscribe->description : old('description') }}"     
              placeholder="{{ trans('admin/main.short_description_placeholder') }}"

>
        {!! !empty($page) ? $page->content : old('content')  !!}
    </textarea>
    @error('content')
    <div class="invalid-feedback">
        {{ $message }}
    ...
    <select name="subscribe_category_id" class="form-control">
        ...
        @foreach($categories as $category)
            <option value="{{ $category->id }}" {{ (!empty($subscribe) && $subscribe->category_id == $category->id) ? 'selected' : '' }}>
                {{ $category->title }}
            </option>
        @endforeach
    </select>
