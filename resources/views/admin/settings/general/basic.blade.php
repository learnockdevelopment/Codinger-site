...
<div class="form-group">
    <label>{{ trans('admin/main.whatsapp_floating_button') }}</label>
    <input 
        type="text" 
        name="value[whatsapp_floating_button]" 
        value="{{ (!empty($itemValue) and !empty($itemValue['whatsapp_floating_button'])) ? $itemValue['whatsapp_floating_button'] : old('whatsapp_floating_button') }}" 
        class="form-control" 
        placeholder="{{ trans('admin/main.enter_whatsapp_number') }}"
    />
    <small class="form-text text-muted">
        {{ trans('admin/main.whatsapp_button_hint') }}
    </small>
</div>
...
<div class="form-group">
    <label class="input-label" for="mobile">{{ trans('auth.default_country') }}:</label>
    @php
    ...
    <select name="value[country_code]" class="form-control select2">
        @foreach(getCountriesMobileCode() as $country => $code)
            <option value="{{ $code }}" 
                    @if((!empty($itemValue) and !empty($itemValue['country_code'])) and $itemValue['country_code'] == $code) selected @endif 
                >
                {{ $country }}
            </option>
        @endforeach
    ...
    <label class="custom-switch pl-0">
        ...
        <input type="checkbox" name="value[user_multi_currency]" id="multiCurrency" value="1" {{ (!empty($itemValue) and !empty($itemValue['user_multi_currency']) and $itemValue['user_multi_currency']) ? 'checked="checked"' : '' }} class="custom-switch-input"/>
        <span class="custom-switch-indicator"></span>
        <label class="custom-switch-description mb-0 cursor-pointer" for="multiCurrency">{{ trans('update.user_multi_currency') }}</label>
    </label>
    <div class="text-muted text-small mt-1">{{ trans('update.user_multi_currency_hint') }}</div>
</div>
