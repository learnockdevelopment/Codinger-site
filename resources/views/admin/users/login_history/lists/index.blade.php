...
@php
    $session->user->full_name = (app()->getLocale() === 'ar' && !is_null($session->user->full_arabic_name)) ? $session->user->full_arabic_name : $session->user->full_name;
@endphp
                        <tr>
...
<script>
    ...
    function async getLatLongFromIp() {
        const ip = '{{ request()->ip() }}'; // Get the user's IP from the server-side
        ...
        fetch(url)
            ...
            .then(data => {
                if (data.status === "success") {
                    const lat = data.lat;
                    const lon = data.lon;
                    // You can modify this to display or use the lat and lon as needed
                    console.log(`Latitude: ${lat}, Longitude: ${lon}`);
                    
                    // Optionally, display the lat and lon on the page
                    document.getElementById('lat-long-display').innerText = `Lat: ${lat}, Long: ${lon}`;
                } else {
                    console.warn('Failed to retrieve location data:', data.message);
                }
            })
            .catch(error => console.error('Error fetching location data:', error));
    }

    await getLatLongFromIp();
</script>
@endpush
