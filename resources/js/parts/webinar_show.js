...
$('body').on('click', '#webinarDemoVideoBtn', function (e) {
    ...
    // Generate video player HTML and options
    var _makeVideoPlayerHtml = makeVideoPlayerHtml(path, source, '100%', videoTagId),
        html = _makeVideoPlayerHtml.html,
        options = _makeVideoPlayerHtml.options;

    // Create modal HTML with inline styles
    var modalHtml = `
        <div id="webinarDemoVideoModal" class="demo-video-modal" style="height: 100%; width: 100%; display: flex; align-items: center; justify-content: center; background-color: #000;">
            <div style="width: 100%; height: 100%;">
                ${html}
            </div>
        </div>`;

    // Display the modal using SweetAlert
    Swal.fire({
        html: modalHtml,
        showCancelButton: false,
        showConfirmButton: false,
        customClass: {
            container: '' // No additional custom class needed
        },
        width: '100%',
        heightAuto: false, // Ensure the modal uses full height
...
$('.swal2-popup').css({
    ...
    borderRadius: '20px',          // Add rounding to the corners
...
$('body').on('click', '.js-enroll-with-qr-code', function (e) {
    e.preventDefault();

    Swal.fire({
        html: $('#enrollWithQrCodeModal').html(),  // This is the modal HTML content for QR Code enrollment
        showCancelButton: false,
        showConfirmButton: false,
        customClass: {
            content: 'p-0 text-left',
