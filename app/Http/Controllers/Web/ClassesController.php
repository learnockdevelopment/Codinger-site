...
class ClassesController extends Controller
{
    ...
    if (!empty($activeSubscribe) && $activeSubscribe->category) {
        $category = $activeSubscribe->category;

        $type = $request->get('type');
        
        if (!empty($type) && is_array($type) && in_array('bundle', $type)) {
            $webinarsQuery = Bundle::where('bundles.status', 'active')
                ->where('category_id', $category->id); // Filter bundles by category
            ...
            $webinarsQuery = Webinar::where('webinars.status', 'active')
                ->where('private', false)
                ->where('category_id', $category->id); // Filter webinars by category
        }
