...
class SubscribeFlowController extends Controller
{
    public function index(Request $request,$subscribe_id)
    {
        ...
        if (in_array(HomeSection::$subscribes, $selectedSectionsName)) {
            $subscribes = Subscribe::where('id',$subscribe_id)->get();
        ...
        $heroSection = (!empty($siteGeneralSettings['hero_section2']) && $siteGeneralSettings['hero_section2'] == "1") ? "2" : "1";
Log::info('Determined hero section:', ['hero_section' => $heroSection]);

$heroSectionData = getHomeHeroSettings($heroSection);
Log::info('Fetched hero section data:', ['hero_section_data' => $heroSectionData]);
      ...
      $subscribe = Subscribe::find($subscribe_id); // Get the subscribe record

      $mainCategories = Category::whereNull('parent_id')
                          ->where('id',$subscribe->category_id)
                         ->orderBy('order')
                         ->get()
                         ->toArray();
          $courses = Webinar::all();
...
// Get the general settings
    $settings = Setting::where('name', 'general')->first();
    
    // Initialize default values
    $allLanguages = getLanguages(); // Assuming this helper exists
    $selectedLanguages = [];
    
    if ($settings && !empty($settings->value)) {
        $itemValue = json_decode($settings->value, true);
        
        // Filter and validate selected languages
        if (!empty($itemValue['user_languages']) && is_array($itemValue['user_languages'])) {
            $selectedLanguages = array_intersect(
                array_keys($allLanguages), // Only allow keys that exist in getLanguages()
                $itemValue['user_languages']
            );
        }
    }
        ...
        $data = [
            'selectedLanguages' => $selectedLanguages, // Cleaned list of selected language keys
            'pageTitle' => $pageTitle,
        ...
        return view(getTemplate() . '.pages.subscribe', $data);
    }
