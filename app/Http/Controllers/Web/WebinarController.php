...
if (auth()->check()) {
    ...
    $currentSessionId = session()->getId(); // or retrieve it based on your application's logic
if($user){

$query = UserLoginHistory::query()
    ->where('user_id', $user->id)  // Filter by the logged-in user
    ->where('session_id', $currentSessionId) // Filter by the current session ID
    ->select(DB::raw('*, ST_AsText(location) as location'));

// Execute the query
$results = $query->get(); // This will execute the query

// Log the query and bindings
Log::info("Executed Query: " . $query->toSql());  // Logs the SQL statement
Log::info("Query Bindings: " . json_encode($query->getBindings()));  // Logs the bindings
Log::info("Query Result: " . json_encode($results));  // Logs the query result
$browserDetected = false;
// Check if there are results before accessing the first element
if ($results->isNotEmpty()) {
    // Get the browser info from the first result
    $browserInfo = $results[0]->browser; // Access the browser property directly

    // Check if the MAC address is included in the browser info
    if (str_contains($browserInfo, "N/A")) {
        // Log the error message
      $browserDetected = true;
        
    }
// Check if the browser is detected
if ($browserDetected) {
    // Set a flash message in the session
    session()->flash('error', "Please use our apps.");

}

} else {
    // Log that no results were found
    Log::info("No login history found for the user in the current session.");
}
}
...
$category_id = DB::table('category_translations')
    ->where('title', $course->category->title)
    ->value('category_id'); // Directly fetch the category_id
    ...
    {
      Log::info("djgagdsjdig");  // Logs the SQL statement
        $webinar = Webinar::where('slug', $slug)
            ->where('status', 'active')
            ->first();
Log::info($webinar);  // Logs the SQL statement
...
{
    \Log::info("playFile: Called with slug = $slug, file_id = $file_id");
    ...
    if ($webinar) {
        \Log::info("playFile: Webinar found", ['webinar_id' => $webinar->id]);

        if ($this->checkCanAccessToPrivateCourse($webinar)) {
            \Log::info("playFile: User has access to private course");
            ...
            if ($file) {
                \Log::info("playFile: File found", ['file_id' => $file->id]);

                $canAccess = true;

                if ($file->accessibility === 'paid') {
                    $canAccess = $webinar->checkUserHasBought();
                    \Log::info("playFile: File requires purchase. Access check:", ['canAccess' => $canAccess]);
                }
                ...
                if ($canAccess) {
                    ...
                    if (in_array($file->storage, $notVideoSource)) {
                        \Log::info("playFile: File is not a local video, loading interactive view", ['storage' => $file->storage]);
                        ...
                        $filePath = public_path($file->file);
                        \Log::info("playFile: Serving local video file", ['path' => $filePath]);

                        return response()->file($filePath);
                    } else {
                        \Log::warning("playFile: File is not recognized as a valid video format");
                    }
                } else {
                    \Log::warning("playFile: User cannot access paid file");
                }

                if (isset($browserDetected)) {
                    \Log::warning("playFile: browserDetected variable triggered fallback");
                ...
                \Log::warning("playFile: File not found");
            }

        } else {
            \Log::warning("playFile: User cannot access private course");
        }

    } else {
        \Log::warning("playFile: Webinar not found");
    }

    \Log::error("playFile: Aborting with 403");
    abort(403);
}
public function getLesson(Request $request, $slug, $lesson_id)
{
    \Log::info("getLesson: Called with slug = $slug, lesson_id = $lesson_id");

    $user = auth()->check() ? auth()->user() : null;

    if ($user) {
        \Log::info("getLesson: Authenticated user", ['user_id' => $user->id]);
    }
    ...
    if ($course && $this->checkCanAccessToPrivateCourse($course)) {
        \Log::info("getLesson: Course found and access granted", ['course_id' => $course->id]);
        ...
        if ($textLesson) {
            \Log::info("getLesson: Text lesson found", ['lesson_id' => $textLesson->id]);

            $canAccess = $course->checkUserHasBought();
            \Log::info("getLesson: Purchase status", ['canAccess' => $canAccess]);

            if ($textLesson->accessibility === 'paid' && !$canAccess) {
                \Log::warning("getLesson: User has not purchased this lesson");
            ...
            $sequenceContentHasError = !empty($checkSequenceContent) && (
                !empty($checkSequenceContent['all_passed_items_error']) ||
                !empty($checkSequenceContent['access_after_day_error'])
            );

            if ($sequenceContentHasError) {
                \Log::warning("getLesson: Sequence access restriction", ['sequence' => $checkSequenceContent]);

                $toastData = [
                    'title' => trans('public.request_failed'),
                    'msg' => ($checkSequenceContent['all_passed_items_error'] ?? '') . ' ' . ($checkSequenceContent['access_after_day_error'] ?? ''),
                    'status' => 'error'
                ];
                return back()->with(['toast' => $toastData]);
            }
            ...
            if (!empty($course->textLessons) && count($course->textLessons)) {
                ...
                \Log::info("getLesson: Found navigation lessons", [
                    'next_lesson' => optional($nextLesson)->id,
                    'previous_lesson' => optional($previousLesson)->id
                ]);
            }

            if ($nextLesson) {
                $nextLesson->not_purchased = ($nextLesson->accessibility === 'paid' && !$canAccess);
            }
            ...
            \Log::info("getLesson: Returning text lesson view");
            return view(getTemplate() . '.course.text_lesson', $data);
        } else {
            \Log::warning("getLesson: Text lesson not found");
        }

    } else {
        \Log::warning("getLesson: Course not found or access denied");
    }

    \Log::error("getLesson: Aborting with 404");
    abort(404);
}
