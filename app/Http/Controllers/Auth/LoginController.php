...
public function logout(Request $request)
{
    ...
    if ($user) {
        Log::info('User logout initiated.', ['user_id' => $user->id, 'logged_count' => $user->logged_count]);
    } else {
        Log::warning('Logout attempted without an authenticated user.');
    }

    $userLoginHistoryMixin = new UserLoginHistoryMixin();
    $userLoginHistoryMixin->storeUserLogoutHistory($user->id);

    Log::info('User logout history recorded.', ['user_id' => $user->id]);
    ...
    Log::info('User session invalidated.', ['user_id' => $user->id]);

    $request->session()->regenerateToken();
    Log::info('User session token regenerated.', ['user_id' => $user->id]);
    ...
    if (!empty($user) && $user->logged_count > 0) {
        ...
        Log::info('User logged count updated.', ['user_id' => $user->id, 'new_logged_count' => $user->logged_count - 1]);
    }
