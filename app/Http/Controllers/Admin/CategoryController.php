...
public function getWebinarsByCategory($categoryIds)
{
    // Log the incoming category IDs
    Log::info('Fetching webinars for categories: ' . $categoryIds);

    // Log before converting category IDs
    Log::info('Converting category IDs to array');
    // Convert category IDs to an array
    $categoryIds = explode(',', $categoryIds);

    // Log the category IDs after conversion
    Log::info('Converted category IDs: ', ['categoryIds' => $categoryIds]);

    // Fetch webinars related to these categories
    Log::info('Querying webinars related to categories');
    $webinars = Webinar::whereHas('categories', function($query) use ($categoryIds) {
        $query->whereIn('category_id', $categoryIds);
    })
    ->with(['categories']) // Assuming 'categories' is a relationship on the Webinar model
    ->orderBy('id', 'desc')
    ->get();

    // Log the result of the query
    Log::info('Fetched webinars: ', ['webinars' => $webinars]);

    // Return the webinars as JSON
    Log::info('Returning response with webinars');
