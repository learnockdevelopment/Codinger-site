    ...
    public function store(Request $request)
{
    ...
    if (!empty($bundle)) {
        ...
        // Fetch the webinar price
        $webinar = \App\Models\Webinar::find($data['webinar_id']);

        if (!$webinar) {
            return response([
                'code' => 422,
                'errors' => [
                    'webinar_id' => [trans('update.webinar_not_found')]
                ],
            ], 422);
        }

        // Add the webinar price to the bundle price
        $bundle->update([
            'price' => $bundle->price + $webinar->price,
        ]);

        // Create the bundle webinar record
    ...
    public function destroy(Request $request, $id)
{
    ...
    $bundleWebinar = BundleWebinar::find($id);

    if ($bundleWebinar) {
        $bundle = Bundle::find($bundleWebinar->bundle_id);
        $webinar = \App\Models\Webinar::find($bundleWebinar->webinar_id);

        if ($bundle && $webinar) {
            // Calculate the new bundle price, ensuring it doesn't go below zero
            $newPrice = $bundle->price - $webinar->price;
            $bundle->update([
                'price' => max(0, $newPrice), // Prevent negative price
            ]);
        }
