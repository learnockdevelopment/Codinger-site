...
$webinar = Webinar::create([
    ...
    'created_at' => time(),
    'updated_at' => time(),
    'fake_counter' => $data['fake_counter'] ?? 0, // Add fake_counter with a default value of 0
    'instructor_money' => $data['instructor_money'],
    'money_back_days' => $data['money_back_days'] ?? null, // Add money_back_days
]);
    ...
    public function update(Request $request, $id)
    {
        ...
        // Log the data that will be passed to the update method
Log::info('Webinar update data:', [
    'slug' => $data['slug'],
    ...
    'points' => $data['points'] ?? null,
    'message_for_reviewer' => $data['message_for_reviewer'] ?? null,
    'commission_rate' => $data['commission_rate'] ?? null,   // Log commission_rate
    'commission_type' => $data['commission_type'] ?? null,   // Log commission_type
    'status' => $data['status'],
    'updated_at' => time(),
]);
...
$webinar->update([
    ...
    'points' => $data['points'] ?? null,
    'message_for_reviewer' => $data['message_for_reviewer'] ?? null,
    'commission_rate' => $data['commission_rate'] ?? null,   // Add commission_rate
    'commission_type' => $data['commission_type'] ?? null,   // Add commission_type
    'commission_for' => $data['commission_for'] ?? null,   // Add commission_for
    'status' => $data['status'],
    'fake_counter' => $data['fake_counter'] ?? 0, // Add fake_counter, default to 0 if not provided
  'instructor_money' => $data['instructor_money'],
  'money_back_days' => $data['money_back_days'] ?? null, // Add money_back_days
    'updated_at' => time(),
]);
