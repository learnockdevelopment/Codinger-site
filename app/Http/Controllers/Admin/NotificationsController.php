...
public function index()
{
    ...
    Log::info("Starting to fetch notifications for admin.");

    // Check if the user has the 'admin_notifications_list' permission
    ...
    Log::info("Executing query to fetch notifications for user ID: 1, ordering by 'created_at' in descending order.");
    ...
    // Log the fetched notifications data (before returning it)
    Log::info("Fetched notifications for user ID: 1 - Notifications count: " . $notifications->count());

    // Optional: Log the content of the notifications for debugging (before translation)
    Log::info("Notifications data (before translation): " . json_encode($notifications->items()));

    // Translate the title and message of each notification
    foreach ($notifications as $notification) {
        try {
            $notification->title = $google->translate($notification->title);
            $notification->message = $google->translate($notification->message);
        } catch (\Exception $e) {
            Log::error("Translation failed for notification ID: " . $notification->id . " Error: " . $e->getMessage());
        }
    }

    // Log the translated notifications data (after translation)
    Log::info("Notifications data (after translation): " . json_encode($notifications->items()));
    ...
    // Log that the view is being returned with the data
    Log::info("Returning the view with notifications data.");
...
foreach ($notifications as $notification) {
        try {
            Log::info("Translating notification ID: " . $notification->id);
            $notification->title = $google->translate($notification->title);
            $notification->message = $google->translate($notification->message);
            Log::info("Translated notification ID: " . $notification->id . " title and message.");
        } catch (\Exception $e) {
            Log::error("Translation failed for notification ID: " . $notification->id . " Error: " . $e->getMessage());
        }
    }
