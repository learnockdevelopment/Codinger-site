    ...
    public function store(Request $request)
{
    ...
    $this->authorize("panel_support_create");
    \Log::info('Authorization passed for user: ' . auth()->user()->id);

    $user = auth()->user();
    \Log::info('Authenticated user:', ['id' => $user->id, 'name' => $user->full_name]);
    ...
    unset($data['type']);
    \Log::info('Type removed from data.');
    ...
    \Log::info('Support ticket created.', ['support_id' => $support->id]);
    ...
    \Log::info('Support conversation created.');

    if (!empty($data['webinar_id'])) {
        $webinar = Webinar::findOrFail($data['webinar_id']);
        \Log::info('Webinar found:', ['webinar_id' => $webinar->id]);

        $notifyOptions = [
            '[c.title]' => $webinar->title,
            '[u.name]' => $user->full_name
        ];
        \Log::info('Sending notification to webinar teacher.', $notifyOptions);
    ...
    if (!empty($data['department_id'])) {
        ...
        \Log::info('Sending notification to admin.', $notifyOptions);
        sendNotification('support_message_admin', $notifyOptions, 1); // for admin
    }

    $url = '/panel/support';
    \Log::info('Setting redirect URL to /panel/support.');

    if (!empty($data['department_id'])) {
        $url = '/panel/support/tickets';
        \Log::info('Updated redirect URL to /panel/support/tickets.');
    }
...
public function automaticTicket(Request $request)
{
    \Log::info('Entering store method.');
    ...
    $response = [
        ...
        'support_id' => $support->id,
        'redirect_url' => !empty($data['department_id']) ? '/panel/support/tickets' : '/panel/support',
    ];
