 ...
 public function show($id)
{
    ...
    $user = apiAuth();
    Log::info('User authenticated', ['user' => $user ? $user->id : 'Guest']);
    ...
    if (!empty($webinar)) {
        Log::info('Webinar fetched', ['webinar' => $webinar]);

$category_id = DB::table('category_translations')
    ->where('title', $webinar->category->title)
    ->value('category_id'); // Directly fetch the category_id

if ($category_id) {
    Log::info('Category ID retrieved', ['category_id' => $category_id]);
} else {
    Log::warning('Category translation not found', ['category' => $webinar->category->title]);
}

        Log::info('Webinar is not empty', ['webinar_id' => $webinar->id]);

        $data = $webinar->details;
        $data['category_id'] = $category_id; // Add category_id to the data

        Log::info('Webinar details retrieved', ['details' => $data]);

        $cashbackRules = null;

        if (!empty($data["price"]) && getFeaturesSettings('cashback_active') && (empty($user) || !$user->disable_cashback)) {
            Log::info('Cashback conditions met', [
                'price' => $data["price"],
                'cashback_active' => getFeaturesSettings('cashback_active'),
                'disable_cashback' => $user ? $user->disable_cashback : 'Guest'
            ]);

            $cashbackRulesMixin = new CashbackRules($user);
            $cashbackRules = $cashbackRulesMixin->getRules('courses', $data["id"], $data["type"], null, null);
            Log::info('Cashback rules calculated', ['cashback_rules' => $cashbackRules]);
        } else {
            Log::info('Cashback conditions not met');
        }

        $data["cashbackRules"] = $cashbackRules;
        Log::info('Final data prepared', ['data' => $data]);

        return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), $data);
    }

    Log::warning('Webinar not found or invalid', ['id' => $id]);
...
{
    Log::info('Brief function called', ['single' => $single]);

    if ($single) {
        Log::info('Single webinar mode, converting to collection');
        $webinars = collect([$webinars]);
    }

    $user = apiAuth();
    Log::info('Authenticated user', ['user' => $user ? $user->id : null]);

    $webinars = $webinars->map(function ($webinar) use ($user) {
        Log::info('Processing webinar', ['webinar_id' => $webinar->id]);

        $hasBought = $webinar->checkUserHasBought($user);
        Log::info('User purchase status', ['hasBought' => $hasBought]);
        ...
        Log::info('Favorite status', ['is_favorite' => $is_favorite]);

        /* live webinar status */
        $live_webinar_status = self::liveWebinarStatus($webinar);
        Log::info('Live webinar status', ['live_webinar_status' => $live_webinar_status]);

        $priceWithDiscount = $webinar->activeSpecialOffer() ? 
    ...
    if ($single) {
        Log::info('Returning single webinar');
        return $webinars->first();
    }

    Log::info('Returning multiple webinars', ['count' => count($webinars)]);
