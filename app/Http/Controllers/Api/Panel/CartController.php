...
{
    \Log::info('Fetching Cart Index', ['user_id' => apiAuth()->id ?? 'unauthenticated']);

    $user = apiAuth();

    if (!$user) {
        \Log::warning('Unauthorized access to cart index.');
    ...
    \Log::info('Carts fetched', ['count' => $carts->count()]);

    $cartt = null;

    if (!empty($carts) && !$carts->isEmpty()) {
        \Log::info('Calculating cart prices.');
        $calculate = $this->calculatePrice($carts, $user);
        \Log::info('Calculated prices', ['calculate' => $calculate]);

        \Log::info('Calculating total cashback amount.');
        $totalCashbackAmount = $this->getTotalCashbackAmount($carts, $user, $calculate["sub_total"]);
        \Log::info('Total cashback amount', ['amount' => $totalCashbackAmount]);

        $hasPhysicalProduct = $carts->where('productOrder.product.type', Product::$physical);
        \Log::info('Physical products in cart', ['count' => count($hasPhysicalProduct)]);

        $deliveryEstimateTime = 0;

        if (!empty($hasPhysicalProduct) && count($hasPhysicalProduct)) {
            \Log::info('Calculating delivery estimate time.');

            foreach ($hasPhysicalProduct as $physicalProductCart) {
                if (
                    !empty($physicalProductCart->productOrder) &&
                    !empty($physicalProductCart->productOrder->product) &&
                    !empty($physicalProductCart->productOrder->product->delivery_estimated_time) &&
            ...
            \Log::info('Calculated delivery estimate time', ['delivery_estimate_time' => $deliveryEstimateTime]);
        }

        if (!empty($calculate)) {
            \Log::info('Preparing cart data for response.');
            ...
            \Log::info('Cart data prepared', ['cart' => $cartt]);
        }
    } else {
        \Log::info('Cart is empty for user', ['user_id' => $user->id]);
    }

    \Log::info('Returning cart index response.');
