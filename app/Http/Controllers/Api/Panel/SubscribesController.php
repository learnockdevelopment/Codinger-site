...
class SubscribesController extends Controller
{
    ...
    public static function createAccountingSystemForSubscribe($orderItem, $userId)
{
...
public function applePay(Request $request)
{
    ...
    Log::info('Apple Pay process started', ['request_data' => $request->all()]);

    $subscribe_id = $request->input('subscribeId');
    Log::info('Received subscribe_id', ['subscribe_id' => $subscribe_id]);

    $subscribe = Subscribe::find($subscribe_id);
    if ($subscribe) {
        Log::info('Found subscription', ['subscribe' => $subscribe->toArray()]);
    } else {
        Log::warning('Subscription not found', ['subscribe_id' => $subscribe_id]);
    }

    $user = apiAuth();
    Log::info('Authenticated users', ['user_id' => $user->id]);

    $activeSubscribe = Subscribe::getActiveSubscribe($user->id);
    if ($activeSubscribe) {
        Log::info('User has an active subscription', ['user_id' => $user->id]);
    ...
    Log::info('Retrieved financial settings', ['financial_settings' => $financialSettings]);

    $tax = $financialSettings['tax'] ?? 0;
    Log::info('Tax rate applied', ['tax' => $tax]);

    $amount = $subscribe->price;
    Log::info('Subscription price', ['amount' => $amount]);

    $taxPrice = $tax ? $amount * $tax / 100 : 0;
    Log::info('Calculated tax price', ['tax_price' => $taxPrice]);
    ...
    Log::info('Order item created or updated', ['order_item' => $orderItem->toArray()]);

    // Call the accounting function
    \Log::info('Calling createAccountingSystemForSubscribe function');
    $this->createAccountingSystemForSubscribe($orderItem, $user->id);
    ...
    Log::info('Returning response data', ['data' => $data]);
