 ...
 * */
function dateTimeFormat($timestamp, $format = 'H:i', $useAdminSetting = true, $applyTimezone = true, $timezone = "Africa/Cairo")
{
    ...
    // Set default timezone to Africa/Cairo if none is provided
    if (empty($timezone)) {
...
function dateTimeFormatForHumans($timestamp, $applyTimezone = true, $timezone = "Africa/Cairo", $parts = 3)
{
    if ($applyTimezone) {
        $timezone = getTimezone();
    }
    ...
    // Log the time zone being used
    Log::info('Timezone used in dateTimeFormatForHumans:', ['timezone' => $timezone]);
...
//this function convert string to Africa/Cairo time zone
function convertTimeToUTCzone($str, $userTimezone = null, $format = false)
{
    ...
    $new_str->setTimeZone(new DateTimeZone('Africa/Cairo'));
