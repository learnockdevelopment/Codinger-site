...
public function makeCourseCertificate($certificate)
{
    ...
    if (!empty($template) && !empty($course)) {
        ...
        // Save the course certificate for the user
        $userCertificate = $this->saveCourseCertificate($user, $course);

        // Get the locale of the application
        $locale = app()->getLocale();

        // Check if there's a translation for the template body in the current locale
        $body = (!empty($template->translate($locale)) && !empty($template->translate($locale)->body))
            ? $template->translate($locale)->body
            : $template->body;

        // Generate the certificate body with required information
        $body = $this->makeBody(
            $template,
            $userCertificate,
            $user,
        ...
        // Return the view with the generated body content
        return view('admin.certificates.create_template.show_certificate', $data);
    }

    // If no template or course found, show an error message
    $toastData = [
