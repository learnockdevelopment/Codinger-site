...
public function getLatestSaleForWebinar($webinarId)
{
    return Sale::where('buyer_id', $this->id)
               ->where('webinar_id', $webinarId)
               ->whereNull('refund_at')
               ->latest('created_at') // Or use 'updated_at' based on your requirement
               ->first();
}
  ...
  private function setUpGoogleTranslate(): GoogleTranslate
    {
        ...
        return $google->setSource('en')
            ->setTarget(app()->getLocale());
    }
    ...
    public function getUnReadNotifications()
    {
        ...
        // Initialize Google Translate instance
        ...
        // Translate each notification's title and message
        foreach ($notifications as $notification) {
            try {
                // Cache the translation for title and message
                $notification->title = $this->getTranslatedText($google, $notification->title);
                $notification->message = $this->getTranslatedText($google, $notification->message);
            } catch (\Exception $e) {
        ...
        if (!empty($userGroup)) {
            ...
            // Translate group notifications content
        ...
        if ($this->isUser()) {
            ...
            // Translate student notifications content
            foreach ($studentsNotifications as $notification) {
                $notification->title = $this->getTranslatedText($google, $notification->title);
                $notification->message = $this->getTranslatedText($google, $notification->message);
            }
        ...
        // Handle instructor-specific notifications
        if ($this->isTeacher()) {
            ...
            // Translate instructor notifications content
            foreach ($instructorNotifications as $notification) {
                $notification->title = $this->getTranslatedText($google, $notification->title);
                $notification->message = $this->getTranslatedText($google, $notification->message);
            }
        ...
        // Handle organization-specific notifications
        if ($this->isOrganization()) {
            ...
            // Translate organization notifications content
            foreach ($organNotifications as $notification) {
                $notification->title = $this->getTranslatedText($google, $notification->title);
                $notification->message = $this->getTranslatedText($google, $notification->message);
            }
        ...
        if (!empty($userBoughtWebinarsIds)) {
            ...
            // Translate course student notifications content
            foreach ($courseStudentsNotifications as $notification) {
                $notification->title = $this->getTranslatedText($google, $notification->title);
                $notification->message = $this->getTranslatedText($google, $notification->message);
            }
    
            if (!empty($courseStudentsNotifications) && !$courseStudentsNotifications->isEmpty()) {
                $notifications = $notifications->merge($courseStudentsNotifications);
            }
        }
    
        // Return sorted notifications by creation date
        return $notifications->sortByDesc('created_at');
    }
    
    // Helper function to handle caching and translation
    private function getTranslatedText($google, $text)
    {
        $cacheKey = 'notification_translation_' . md5($text);
    
        // Check if the translated text is cached
        $translatedText = cache()->get($cacheKey);
    
        if (!$translatedText) {
            // Perform the translation and cache it
            try {
                $translatedText = $google->translate($text);
                cache()->put($cacheKey, $translatedText, now()->addDays(7)); // Cache for 7 days (you can adjust this)
            } catch (\Exception $e) {
                // Handle translation error if needed
                $translatedText = $text; // Fallback to original text in case of error
            }
        }
