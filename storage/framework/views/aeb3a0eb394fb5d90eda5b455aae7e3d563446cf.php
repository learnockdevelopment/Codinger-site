...
?>
    ...
    <section class="container course-content-section mt-15 <?php echo e($course->type); ?> <?php echo e(($hasBought or $percent) ? 'has-progress-bar' : 'py-3'); ?>">
...
<section class="course-cover-container <?php echo e(empty($activeSpecialOffer) ? 'not-active-special-offer' : ''); ?> ">
    ...
    <video 
        ...
        poster="<?php echo e($course->getImageCover()); ?>" 
        muted 
        loop 
        controls>
        <source src="<?php echo e($course->video_demo_source == 'upload' ? url($course->video_demo) : $course->video_demo); ?>" 
                type="video/<?php echo e(pathinfo($course->video_demo, PATHINFO_EXTENSION)); ?>">
    ...
    <div id="customPlayPauseButton" class="custom-play-button d-flex align-items-center justify-content-center">
        <i class="fas fa-play"></i> <!-- Default to play icon -->
                ...
                <?php else: ?>
                    <img src="<?php echo e($course->getImageCover()); ?>" 
                ...
                <div class="course-content-body user-select-none mt-35 ">
                    <div class="course-body-on-cover text-white bg-white  p-3 rounded-sm"  style="border-bottom: 3px solid var(--primary);">
                        ...
                        <?php if(!empty($course->category)): ?>
                            <span class="d-block font-16 mt-10 text-secondary"><?php echo e(trans('public.in')); ?> <a href="<?php echo e($course->category->getUrl()); ?>" target="_blank" class="text-secondary font-weight-500 text-decoration-underline text-white"><?php echo e($course->category->title); ?></a></span>
                        <?php endif; ?>
                        ...
                        <div class="d-flex align-items-center">
                            ...
                            <span class=" text-secondary ml-10 mt-15 font-14 ">(<?php echo e($course->reviews->pluck('creator_id')->count()); ?> <?php echo e(trans('public.ratings')); ?>)</span>
                        </div>

                        <div class="mt-15">
                            <span class="font-14 text-secondary"><?php echo e(trans('public.created_by')); ?></span>
                            <a href="<?php echo e($course->teacher->getProfileUrl()); ?>" target="_blank" class="text-secondary text-decoration-underline text-white font-14 font-weight-500"><?php echo e($course->teacher->full_name); ?></a>
                        </div>
    ...
    <?php if($ticketStartDate <= $currentDate && $ticketEndDate >= $currentDate): ?> <!-- Check if current date is between start and end -->
        <input class="form-check-input" <?php if(!$ticket->isValid()): ?> disabled <?php endif; ?> type="radio"
               data-discount-price="<?php echo e(handleCoursePagePrice($ticket->getPriceWithDiscount($course->getPrice()))['price']); ?>"
    ...
    <?php endif; ?>
                            ...
                            <div class="mt-20 d-flex flex-column">
                                ...
                                <?php elseif($hasBought or !empty($course->getInstallmentOrder())): ?>
                                    <a href="<?php echo e($shouldRedirectToMobileApp ? '/mobile-app' : $course->getLearningPageUrl()); ?>" target="<?php echo e(session('error') ? '_self' : '_blank'); ?>" class="btn btn-primary">
    <?php echo e(trans('update.go_to_learning_page')); ?>
...
<?php if($canSale): ?>
    <button type="button" class="btn btn-outline-primary mt-20 js-course-qr-payment">
        <?php echo e(trans('update.buy_with_qr_code')); ?>
...
?>
                       ...
                       <?php if($hasBought and !empty($remainingMoneyBackDays) && $remainingMoneyBackDays >= 0 && !empty(getOthersPersonalizationSettings('show_guarantee_text')) && getOthersPersonalizationSettings('show_guarantee_text')): ?>
    <div class="mt-20 d-flex align-items-center justify-content-center text-gray hover-clickable" 
        ...
        <span class="ml-5 font-14">
            <?php echo e(trans('product.guarantee_text', ['days' => $remainingMoneyBackDays])); ?>
                ...
                <div class="rounded-lg shadow-sm mt-35 px-25 py-20 bg-white"  style="border-bottom: 3px solid var(--primary);">
                    ...
                    <div class="mt-30">
                        ...
                        <div class="mt-20 d-flex align-items-center justify-content-between text-gray">
                            ...
                            <span class="font-14"><?php echo e($course->getSalesCount() + $course->fake_counter); ?></span>
                        </div>
...
$(document).on('click', '.js-course-qr-payment', function (e) {
    ...
    Swal.fire({
        title: '<?php echo e(trans('update.qr_code_purchase')); ?>',
        html: ` 
            <div id="qr-reader" style="width: 100%;"></div>
            <input type="text" id="manual-qr-code" class="swal2-input" placeholder="<?php echo e(trans('update.enter_qr_code')); ?>">
        `,
        showCancelButton: true,
        confirmButtonText: '<?php echo e(trans('update.confirm_purchase')); ?>',
        cancelButtonText: '<?php echo e(trans('admin/main.cancel')); ?>',
        didOpen: () => {
            ...
            html5QrCode.start(
                ...
                qrCodeMessage => {
                    ...
                    Swal.update({
                        html: `
                            <p><?php echo e(trans('update.qr_code_detected')); ?></p>
                            <input type="text" id="manual-qr-code" class="swal2-input" value="${qrCodeMessage}" readonly>
                        `
                    });
                },
                errorMessage => {
                    console.log('<?php echo e(trans('update.scanning_error')); ?>: ', errorMessage);
                }
            ).catch(err => {
                console.error('<?php echo e(trans('update.camera_error')); ?>: ', err);
            });
        },
        preConfirm: () => {
            ...
            if (!manualCode) {
                Swal.showValidationMessage('<?php echo e(trans('update.qr_code_required')); ?>');
                return false;
            }
    ...
    function fetchQrCodeData(qrCode) {
        ...
        return fetch('/api/development/qrcode/show', {
            ...
            headers: {
                ...
                'x-api-key': '<?php echo e(env('API_KEY')); ?>' // Get the API key from the .env file
            },
            body: JSON.stringify({ code: qrCode, user_id: '<?php echo e(auth()->id()); ?>' })
        })
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (currentDate > expirationDate) {
                    Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.qr_code_expired')); ?>', 'error');
                    return;
                }

                // Check if the QR code has already been used
                if (qrData.is_used) {
                    Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.qr_code_used')); ?>', 'error');
                    return;
                }

                // Check if bundle_id in the request is different from the one in the QR code
                if (qrData.webinar_ids !== '<?php echo e($course->id); ?>' && qrData.webinar_ids !== null) {
                    Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.bundle_mismatch')); ?>', 'error');
                    return;
                }
// Check if category_id in the QR code is different from the course's category_id
if (qrData.category_id != '<?php echo e($category); ?>' && qrData.category_id !== null) {
    Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.category_mismatch')); ?>', 'error');
    return;
}

                // Show success message (but do not display the QR code data to the user)
                Swal.fire({
                    title: '<?php echo e(trans('update.success')); ?>',
                    text: '<?php echo e(trans('update.valid_qr_code')); ?>',
                    icon: 'success',
                    confirmButtonText: '<?php echo e(trans('update.proceed_with_purchase')); ?>',
                    preConfirm: () => {
                ...
                Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.invalid_qr_code')); ?>', 'error');
            }
        })
        .catch(error => {
            console.error('Error fetching QR code data: ', error);
            Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.network_error')); ?>', 'error');
        });
    }
    ...
    function fetchPurchase(qrCode, qrData) {
        ...
        return fetch('/api/development/qrcode', {
            ...
            body: JSON.stringify({
                ...
                course_id: '<?php echo e($course->id); ?>', // Fallback to bundle_id if it's not available in the QR data
            })
        })
        .then(response => response.json())
        .then(result => {
            ...
            if (result.success) {
                ...
                Swal.fire('<?php echo e(trans('update.success')); ?>', '<?php echo e(trans('update.purchase_success')); ?>', 'success');
            } else {
                Swal.fire('<?php echo e(trans('update.failed')); ?>', '<?php echo e(trans('update.purchase_failed')); ?>', 'error');
            }
        })
        .catch(error => {
...
<script>
    function submitRefundRequest() {
        ...
        Swal.fire({
            title: '<?php echo e(trans("product.confirmation_title")); ?>', // Translated title
            text: '<?php echo e(trans("product.confirmation_text")); ?>',   // Translated text
            icon: 'warning',
            showCancelButton: true,
            confirmButtonText: '<?php echo e(trans("product.confirm_button")); ?>',  // Translated confirm button
            cancelButtonText: '<?php echo e(trans("product.cancel_button")); ?>'    // Translated cancel button
        }).then((result) => {
            ...
            if (result.isConfirmed) {
                ...
                Swal.fire({
                    title: '<?php echo e(trans("product.loading_title")); ?>',
                    text: '<?php echo e(trans("product.loading_text")); ?>',
                ...
                fetch('/panel/support/refund-request', {
                    ...
                    body: JSON.stringify({
                        title: '<?php echo e(trans("product.request_title")); ?>',  // Translated request title
                        type: 'platform_support',
                        department_id: 2,  // Adjust based on your logic
                        webinar_id: <?php echo e($course->id ?? 'null'); ?>,
                        message: '<?php echo e(trans("product.request_message")); ?>'  // Translated message
                    })
                }).then(response => {
                    ...
                    if (data.success) {
                        ...
                        Swal.fire('<?php echo e(trans("product.success_title")); ?>', '<?php echo e(trans("product.success_text")); ?>', 'success');
                    } else {
                        console.error('Refund request submission failed:', data);
                        Swal.fire('<?php echo e(trans("product.error_title")); ?>', '<?php echo e(trans("product.error_text")); ?>', 'error');
                    }
                }).catch(error => {
...
<?php echo $__env->make(getTemplate().'.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/codinger/htdocs/codinger.online/resources/views/web/default/course/index.blade.php ENDPATH**/ ?>