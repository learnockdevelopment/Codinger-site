...
<?php $__env->startPush('scripts_bottom'); ?>
    <script>
        // Toggle the "fixed-show" class and apply inline styles when the collapse button is clicked
$('body').on('click', '#collapseBtn', function () {
    var $tabs = $('.learning-page-tabs');
    $tabs.toggleClass('fixed-show');
    var $navTabs = $('.learning-page .learning-page-tabs .nav-tabs');
    // Toggle the "fixed-show" class
    if ($tabs.hasClass('fixed-show')) {
        // Apply inline styles for the fixed-show state
        $tabs.css({
            'width': '373px',
            'min-width': '373px',
            'right': '0',
        ...
        // Media query styling for small screens (can be handled here or with CSS for better performance)
        if (window.innerWidth <= 767) {  // Adjust the breakpoint as necessary
            $tabs.css({
                'width': '100vw',
                'min-width': '100%'
            });
        }
// Apply display block to .learning-page-tabs-link-text and .tab-content
        $('.learning-page-tabs-link-text, .tab-content').css({
            'display': 'block'
        });

    }else {
            // Remove inline styles when "fixed-show" is removed
            $tabs.removeAttr('style');

            // Remove display style for .learning-page-tabs-link-text and .tab-content
            $('.learning-page-tabs-link-text, .tab-content').removeAttr('style');
        }

    console.log("Collapse button clicked, toggling 'fixed-show' class.");
});

    </script>
<?php $__env->stopPush(); ?><?php /**PATH /home/codinger/htdocs/codinger.online/resources/views/web/default/course/learningPage/components/navbar.blade.php ENDPATH**/ ?>